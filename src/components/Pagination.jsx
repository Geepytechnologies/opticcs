...
const Pagination = ({ currentpage, setCurrentpage, pages }) => {
    ...
    const numOfPages = Array.from({ length: pages }).map((_, index) => index + 1);
    const pageleft = () => {
        if (currentpage !== 1) {
            ...
            <div className="flex items-center gap-3">
                ...
                <div className='mx-3 flex items-center justify-center gap-3'>
                    {numOfPages.slice(initialPage, finalPage).map((item, index) => (
                        <div key={index} className=''><div onClick={() => setCurrentpage(item)} className={`w-7 h-7 cursor-pointer flex items-center justify-center ${currentpage == item && 'bg-primary90 rounded-full text-white'}`}>{item}</div></div>
                    ))}
