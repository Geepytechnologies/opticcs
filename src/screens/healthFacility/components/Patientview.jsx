...
const Patientview = () => {
    ...
    const getvisitdates = async () => {
        try {
            ...
            setReturnvisitparam({ id: res.data.returnvisit[0].id, date: res.data.returnvisit[0].returnvisit_date })
            setDates(res.data)
        } catch (error) {
    ...
    return (
        <div className='bg-primary10 min-h-screen'>
            <div className='flex items-center justify-between  py-[40px] mx-4'>
                {/* arrow */}
                <div onClick={() => navigate(-1)} className='flex cursor-pointer flex-1 items-center gap-2 font-[600] text-[24px]'>
                    <IoReturnUpBackOutline />
                    <p>Patient View</p>

                </div>
                {/* name section */}
                {data && <div className='border-[1.2px] rounded-[10px] flex flex-[3] items-center p-2 justify-center'>
                    <div className='flex flex-col gap-1 items-center justify-center'>
                        ...
                        <p className='font-[600] text-[24px]'>{data?.firstname.charAt(0).toUpperCase() + data?.firstname.slice(1) ?? ""}</p>
                        ...
                        (currentpage == "firstvisit") ?
                            <div className='flex flex-col gap-2 mt-6'>
                                ...
                                {dates?.firstvisit.map((item, index) => (
                                    <p key={index} className='bg-primary90 p-2 text-center rounded-[10px] text-white'>{moment(item.firstvisit_date).format('yyyy-MM-DD')}</p>
                                ))}
                            </div>
